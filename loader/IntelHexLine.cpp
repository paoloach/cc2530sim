...
IntelHexLine * IntelHexLine::parseLine(const std::string &line) {
    ...
    uint32_t address = std::stoi(line.substr(3,2), nullptr,16);
    if (line.size() <  9 + dataLen+2){
        throw InvalidIntelHexFile("line too short");
    }
    std::vector<uint8_t > data;
    for(int i=0; i < dataLen; i++){
        data.push_back(std::stoi(line.substr(11+i*2, 2), nullptr, 16));
    }

    switch (decodeType(line.substr(7,2))){
        case Data:
            return new IntelHexData(address, std::move(data));
    }
